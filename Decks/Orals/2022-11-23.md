...
%
Definition 2.1.1. A ring $R$ with field of fractions $K$ is normal if every element of $K$ which is integral over $R$ (i.e. a root of monic polynomial in $R[x]$ ) is actually in $R$. A variety $X$ is normal if it is irreducible and the local rings $\mathcal{O}_{X, p}$ are normal for all $p \in X$
%
definition
---
